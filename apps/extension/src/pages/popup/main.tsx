...
function formatVoteSourceCount(count: number, noun: string): string {
  return `${count} ${noun}${count === 1 ? '' : 's'}`;
}

function getGovernanceVoteSourceStatusPills(summary: GovernanceVoteSourceSummary): Array<{
  label: string;
...
function getGovernanceVoteActionNote(summary: GovernanceVoteSourceSummary): string | null {
...
function getGovernanceProposalTimeMeta(
  ...
  if (!proposal.votingEndsAt) {
    return {
      ...
      badgeTone: hasRemainingVotes ? (hasAnyVotes ? 'warning' : 'success') : hasAnyVotes ? 'success' : 'neutral',
      metaText: null,
      noteText: null,
  ...
  function renderGovernanceProposalCard(
    ...
    return (
      <div key={proposal.proposalId} className="governance-proposal-card">
        ...
        {voteSourceStatusPills.length > 0 ? (
          <div className="governance-vote-source-pills">
            {voteSourceStatusPills.map((pill) => (
              <StatusPill key={`${proposal.proposalId}:${pill.label}`} tone={pill.tone}>
                {pill.label}
              </StatusPill>
            ))}
          </div>
        ...
        {canVoteNow ? (
          <>
            {voteActionNote ? <p className="muted governance-proposal-note">{voteActionNote}</p> : null}
      ...
      <>
        ...
        <Tabs.Root value={activeHomeTab} onValueChange={(value) => setHomeTab(value as HomeTab)}>
          <Tabs.List className="content-tabs wallet-home-tabs" aria-label="Wallet content">
            ...
            {isSolanaChain ? (
              <Tabs.Trigger className="content-tab content-tab-primary" value="governance">
      ...
      <>
        {renderSettingsSection({
          ...
          <div className="stack">
            ...
            <div className="stack">
              <div className="settings-row">
                <span className="muted">Grape Verification Gate</span>
                <strong>Disabled</strong>
              </div>
              <p className="muted">The global Grape verification requirement is turned off for now.</p>
            </div>
            <div className="stack">
              <div className="settings-row">
