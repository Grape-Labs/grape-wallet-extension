...
describe('permissions', () => {
  it('grants and revokes per-origin permissions', () => {
    ...
    const granted = grantPermissions(initial, 'https://example.com', ['solana:accounts', 'sui:accounts', 'monad:sign', 'ethereum:accounts'], {
      title: 'Example'
    });
    ...
    expect(hasPermission(granted, 'https://example.com', 'ethereum:accounts')).toBe(true);
    ...
    expect(hasPermission(revoked, 'https://example.com', 'ethereum:accounts')).toBe(false);
  });
});
